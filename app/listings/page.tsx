...
/** Kandidat bucket foto (diperluas) */
const IMAGE_BUCKETS = [
  "Listing_image",
  "listing-images",
  "listing_images",
...
/** Ambil 1 foto pertama untuk listingId dari bucket yang tersedia (fallback diperluas) */
async function getFirstImageFromBuckets(listingId: string): Promise<string | null> {
  for (const bucket of IMAGE_BUCKETS) {
    const { data: files, error } = await supabase.storage
      .from(bucket)
      .list(listingId, { limit: 100, sortBy: { column: "name", order: "asc" } });

    if (error || !files || files.length === 0) continue;

    // jangan terlalu ketat: pilih file gambar jika ada, jika tidak ambil pertama apa adanya
    const candidate =
      files.find((f) => /\.(jpe?g|png|webp|gif)$/i.test(f.name)) ?? files[0];

    const { data } = supabase.storage.from(bucket).getPublicUrl(`${listingId}/${candidate.name}`);
    if (data?.publicUrl) return data.publicUrl;
  }
  return null;
}

/** Fallback dari tabel listing_images (jika ada) */
async function getFirstImageFromTable(listingId: string): Promise<string | null> {
  ...
  // file_path contoh: "Listing_image/<id>/nama.jpg"
...
/** Jika tidak ada foto, coba ambil 1 video agar kartu tidak kosong */
async function getFirstVideoUrl(listingId: string): Promise<string | null> {
  ...
  const first = files.find((f) => /\.(mp4|webm|mov|m4v)$/i.test(f.name)) ?? files[0];
  const { data } = supabase.storage.from(VIDEO_BUCKET).getPublicUrl(`${listingId}/${first.name}`);
  return data?.publicUrl ?? null;
}
...
export default async function ListingsPage() {
  ...
  const previews = await Promise.all(
    rows.map(async (row) => ({ id: row.id, preview: await getPreviewUrl(row.id) }))
  );
